...
def handle_stop_signal(signum, frame):
    """Xử lý ngắt tiến trình tức thời khi nhận tín hiệu dừng từ người dùng."""
    logger.info("Received stop signal (%s). Terminating download cleanly.", signum)
    send_ipc({
        "stage": "stopped",
        "status": "stopped",
        "progress": 0,
        "message": "🛑 Tiến trình quét và tải đã bị dừng bởi người dùng."
    })
    sys.exit(0)
...
def download_single_item(
    ...
    if highest_quality:
        ...
        format_spec = "best[ext=mp4]/bestvideo[height<=720]+bestaudio/best"
    ...
    # Proxy Configuration
    if proxy_url and proxy_url.strip() and proxy_url.lower() != "direct":
        ydl_opts['proxy'] = proxy_url.strip()
        logger.info("Using Proxy for download: %s", proxy_url)

    # Headers and Cookie Configuration
    ...
    if cookie_str and cookie_str.strip():
        ...
        logger.info("Injecting custom Cookie into request headers.")
...
def main() -> None:
    import sys
    import io
    # Register signal handlers for clean stop
    try:
    ...
    parser.add_argument("--out_dir", type=str, default="downloads", help="Output directory path")
    parser.add_argument("--cookie", type=str, default="", help="Custom raw cookie string for TikTok/Douyin")
    parser.add_argument("--proxy", type=str, default="", help="Custom proxy URL (http://, socks5://)")
    parser.add_argument("--highest_quality", action="store_true", default=True, help="Enforce highest possible video resolution")
    parser.add_argument("--no_zip", action="store_true", default=False, help="Save MP4 directly without packaging ZIP")
    parser.add_argument("--zip_name", type=str, default="CreatorOS_Batch_Export.zip", help="ZIP filename")

    args = parser.parse_args()

    # Chuẩn hóa đường dẫn tuyệt đối cho output directory
    normalized_out_dir = os.path.abspath(os.path.expanduser(args.out_dir))
    os.makedirs(normalized_out_dir, exist_ok=True)
    logger.info("Destination folder verified: %s", normalized_out_dir)
    print(f"[PATH CONFIRM] 📁 Thư mục lưu trữ video MP4: {normalized_out_dir}", flush=True)
    ...
    send_ipc({
        ...
        "message": f"Bắt đầu tải {total} video .MP4 -> Lưu tại thư mục: {normalized_out_dir}"
    })

    completed_items = []
    for idx, item in enumerate(items):
        res = download_single_item(
            item=item,
            out_dir=normalized_out_dir,
        ...
        send_ipc({
            ...
            "out_dir": normalized_out_dir,
            "message": f"Đã lưu video [{item.get('videoId') or item.get('id')}] vào: {normalized_out_dir}"
        })

    # Dọn dẹp sạch sẽ toàn bộ thư mục đích, chỉ giữ .mp4
    cleanup_non_mp4_files(normalized_out_dir)

    final_zip = None
    count = len([f for f in os.listdir(normalized_out_dir) if f.lower().endswith(".mp4")])

    if not args.no_zip:
        # Đóng gói toàn bộ video vào tệp ZIP CreatorOS_Batch_Export.zip nếu không bật --no_zip
        send_ipc({
            "stage": "zipping",
            "status": "zipping",
            "progress": 95,
        ...
        zip_path = os.path.join(normalized_out_dir, args.zip_name)
        final_zip, count = package_zip(normalized_out_dir, zip_path)
    ...
    send_ipc({
        ...
        "zip_path": final_zip,
        "total_files": count,
        "message": f"🎉 Tải thành công toàn bộ {total} video .MP4 trực tiếp vào: {normalized_out_dir}"
    })
